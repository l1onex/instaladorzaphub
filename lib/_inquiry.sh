...
get_link_git() {
  ...
  print_banner
  printf "${WHITE} 💻 Insira o link do GITHUB do seu ZapHub que deseja instalar:${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " link_git
}
...
inquiry_options() {
  ...
  print_banner
  printf "${WHITE} 💻 Bem vindo(a) ao Gerenciador ZapHub - Atendimento sem limites, selecione abaixo a proxima ação!${GRAY_LIGHT}"
  printf "\n\n"
  printf "   [0] Instalar ZapHub\n"
  printf "   [1] Atualizar ZapHub\n"
  printf "   [2] Deletar ZapHub\n"
  printf "   [3] Bloquear ZapHub\n"
  printf "   [4] Desbloquear ZapHub\n"
  printf "   [5] Alter. dominio ZapHub\n"
  printf "\n"
  read -p "> " option
